...
function Content() {
  ...
  const [currentSymbol, setCurrentSymbol] = useState(symbols[Math.floor(Math.random() * symbols.length)][0]); // 画面に表示する記号
  ...
  const handleKeyPress = useCallback((e) => {
    ...
    if (KeyName === ' ') {
      ...
      setCurrentSymbol(symbols.splice(Math.floor(Math.random() * symbols.length), 1)[0]);
      setSymbols(symbols);
  ...
  }, [currentSymbol, incorrectNumber, isSpaceKeyDowned, restSymbolNumber,symbols]);
    ...
    <>
      {
          ...
          ? restSymbolNumber
            ?
              <div>
                  <PleaseKeyDown>表示された数字または記号のキーを押してください</PleaseKeyDown>
